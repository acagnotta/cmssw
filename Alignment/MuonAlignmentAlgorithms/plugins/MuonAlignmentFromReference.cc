...
MuonAlignmentFromReference::MuonAlignmentFromReference(const edm::ParameterSet& cfg, edm::ConsumesCollector& iC)
    : AlignmentAlgorithmBase(cfg, iC),
      ...
      // m_propToken(iC.esConsumes(edm::ESInputTag("", "SmartPropagatorAnyRKOpposite"))),
        ...
        if ((int)chamberIds.size() >= m_minNCrossedChambers) {
          ...
          for (std::vector<DetId>::const_iterator chamberId = chamberIds.begin(); chamberId != chamberIds.end();
            ...
            // CSC
              ...
              m_counter_csc++;
              //std::cout<<"chamber id in cscelif: "<< csc->chamberAlignable()->geomDetId().rawId()<<std::endl; //MK
              if (csc != nullptr) {
                m_counter_cscvalid++;
                Alignable* ali = csc->chamberAlignable();
                CSCDetId id(ali->geomDetId().rawId());
                //MK FIX for 4-layer chambers
                bool isProblematic = false;
                std::string currentChamber = std::to_string(id.endcap()) + " " + std::to_string(id.station()) + " " + std::to_string(id.ring()) + " " + std::to_string(id.chamber());
                std::vector<std::string> problematicChambers= {
                                                "-1 1 1 15", "-1 1 4 15", 
                                                "-1 1 1 1", "-1 1 4 1", 
                                                "1 1 1 31", "1 1 4 31",
                                                "-1 4 2 4", 
                                                "1 1 1 33", "1 1 4 33"}; //list of CSC chambers for which TBMA will use >= layers to process for 2022D dataset
                //std::cout<<"csc numhits: "<< csc->numHits()<< " csc m_minCSCHits: "<< m_minCSCHits<<std::endl;
               for (const std::string& str : problematicChambers) {
                ...
                //End of Fix
                if (csc->numHits() >= 5 || (isProblematic && csc->numHits() >= 4)) { 
                  m_counter_cschits++;
                  Alignable* ali = csc->chamberAlignable();
                  CSCDetId id(ali->geomDetId().rawId());

                  //std::cout<<"chamber before: "<< id.rawId()<<std::endl; //MK
                  if (m_combineME11 && id.station() == 1 && id.ring() == 4)
                    ali = m_me11map[ali];
